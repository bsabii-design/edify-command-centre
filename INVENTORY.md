...
- **Task-scoped intent routing** (`handleSupplierMessage`): a message inside an open new-supplier draft touches the draft only when it carries supplier info (email/days/cut-off/minimum), a supplier command, or a switch. A message with no readable detail that matches another scenario (e.g. "Why is GP% down…") is treated as a **separate question** — the draft is left untouched (still in Continue), a one-line system message "This looks like a separate question. I'll keep the supplier draft unchanged." stays in the supplier thread, the question never appears below the form, and it branches to its own chat (`onSwitch`) — GP lands in Chats → Questions, the supplier stays in Linked to work. A related message updates the fields and confirms **specifically** ("Updated the draft — Delivery days: Mon, Thu · Cut-off: 16:00."), never a bare "Updated the draft." (Ambiguous-choice branch from the spec is optional and not built.)
...
- **Shared page controls** (one text style + `--ctl-h: 30px`): primary CTA (`.page-action`) 13 Medium white, height 30; Search 13 Regular, muted placeholder, height 30, bordered `--border-strong` pill. **Filters (`.ptab`) read like Search** — every tab is a bordered neutral pill (same border/height/radius as `.search-inline`), transparent + muted by default; the selected tab uses the hover treatment (subtle `--surface-2` fill + darker Medium text). Sidebar Search is a separate system, untouched.
