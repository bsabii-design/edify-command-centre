...
- Journal keeps date grouping (Today/…) and filters All/Confirmed/System handled/Flagged/Dismissed; columns Event · Area · Time. Its table is **full-bleed**: header/description/filters sit at the page gutter, but the header divider, group labels and row hover run edge-to-edge across the main area while row text keeps the gutter (Event on the title's left anchor, Time near the right gutter). No leading row icons — all event titles share one left anchor; issues read through copy and a restrained red on a leading "£X higher" fragment only (never the whole row/title). A future icon system would need its own fixed-width status column; not built here.
