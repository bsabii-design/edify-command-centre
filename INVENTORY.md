...
- Journal keeps date grouping (Today/…) and filters All/Confirmed/System handled/Flagged/Dismissed; columns Event · Area · Time. Its table is **full-bleed**: header/description/filters sit at the page gutter, but the header divider, group labels and row hover run edge-to-edge across the main area while row text keeps the gutter (Event on the title's left anchor, Area centred in its column, Time near the right gutter). No leading row icons — all event titles share one left anchor; issues read through copy and a restrained red on a leading "£X higher" fragment only (never the whole row/title). A future icon system would need its own fixed-width status column; not built here.
- **Journal is the reference list pattern** (to be rolled out to the Spaces later): compact ~52px rows (`sp-2` vertical padding), title 13 Medium ink + subtitle 13 Regular `--text-2` sitting tight (`sp-0` gap), soft-but-visible full-row hover `--surface-2` that hugs the row height, and a "Today" section label at the row gutter with a generous gap from the header divider and a smaller gap to the first row.
