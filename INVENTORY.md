...
- **StatusChip** (`Page.jsx`) — one compact pill (20px, `r-full`, 11 Medium, no border/shadow, not clickable), **mostly monochrome**: `neutral` grey bg + ink text (Confirmed), `muted` grey bg + secondary text (Completed/Received/Resolved/Matched), `waiting` warm-neutral bg `--chip-warm-bg` + muted text (Waiting for supplier/approval — never red), `attention` light-red `--red-bg` + red `--chip-red-text` #E12E07 (Due now/Needs check-in/Needs review/Late/Overdue). No blue/green/amber — colour is reserved for attention; meaning comes from the label. Only the chip is coloured. Inventory **variance** stays plain text (muted `—` / red `−2 L`).
